...
exports.getLoaispById = (req, res) => {
    const { id } = req.params;
    db.query('SELECT * FROM loaisp WHERE MaLoai = ?', [id], (err, results) => {
        if (err) {
...
exports.createLoaisp = (req, res) => {
    ...
    db.query(
        'INSERT INTO loaisp (MaLoai, TenLoai, TrangThai) VALUES (?, ?, ?)', [MaLoai, TenLoai, TrangThai],
        (err, results) => {
            if (err) {
                console.error('Lỗi truy vấn cơ sở dữ liệu:', err);
                res.status(500).send('Lỗi truy vấn cơ sở dữ liệu');
...
exports.updateLoaisp = (req, res) => {
    ...
    db.query(
        'UPDATE loaisp SET TenLoai = ?, TrangThai = ? WHERE MaLoai = ?', [TenLoai, TrangThai, id],
        (err, results) => {
            if (err) {
                console.error('Lỗi truy vấn cơ sở dữ liệu:', err);
                res.status(500).send('Lỗi truy vấn cơ sở dữ liệu');
...
exports.deleteLoaisp = (req, res) => {
    ...
    db.query('DELETE FROM loaisp WHERE MaLoai = ?', [id], (err, results) => {
        if (err) {
            console.error('Lỗi truy vấn cơ sở dữ liệu:', err);
            res.status(500).send('Lỗi truy vấn cơ sở dữ liệu');
        } else if (results.affectedRows === 0) {
